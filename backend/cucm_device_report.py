...
# import MOD_cucm_funcs
# import MOD_db_funcs
# import MOD_device_funcs
...
# ############################################
# ## Get Real-time Info - RisPort - SOAP		 - The query adds round 6% to CPU load
# ## Get real-time info for devices
# #############################################
#
# command = "SelectCmDevice"
# query = {'SelectBy':'Name', 'Status':'Any', 'Class':'Any', 'MaxReturnedDevices':'1000'}
# result = MOD_cucm_funcs.cucm_risport(cmserver_hostname,cmport,soap_user,soap_pass,command,query)
# #print result
#
# ## Measure Script Execution
# print "\n\n--->Runtime After RisPort query = ",datetime.datetime.now()-start
#
# # Update all_devices list according to RIS results
# ## all_devices = mac_address - description - extension - device type
# ## all_devices = mac_address - description - extension - device type - registration status - timestamp
#
# for node in result['SelectCmDeviceResult']['CmNodes']:
# 	if node['Name'] in cmserver_ip:
# 		for device in node['CmDevices']:
# 			#print device
# 			if MOD_cucm_funcs.cucm_is_interering_device(device['Name'],device['Description']):
# 				timestamp = datetime.datetime.fromtimestamp(int(device['TimeStamp'])).strftime('%Y-%m-%d %H:%M:%S')
# 				for i,my_all_device in enumerate(all_devices):
# 					if my_all_device[0] == device['Name']:
# 						if device['Status'] == "Registered":
# 							all_devices[i].append("reg")
# 							all_devices[i].append(timestamp)
# 						elif device['Status'] == "UnRegistered":
# 							all_devices[i].append("unreg")
# 							all_devices[i].append(timestamp)
#
# # Scan all_devieces list and fill "unreg" for those not found at RIS
# for i,device in enumerate(all_devices):
# 	if len(device) == 4:
# 		all_devices[i].append("unreg")
# 		all_devices[i].append(" More than a week")
#
#
#
# #################################
# ## Scan all_devices list and fill "unreg" for those not found at RIS
# ## all_devices = mac_address - description - extension - device type - registration status - timestamp
# ## unreg_devices = mac_address - description - extension - timestamp
#
# unreg_devices = []
# unreg_devices_count = [0]*5
# for my_all_device in all_devices:
# 	if my_all_device[4] == "unreg":
# 		unreg_device = [my_all_device[0], my_all_device[1], my_all_device[2], my_all_device[5]]
# 		unreg_devices.append(unreg_device)
#
# unreg_devices = sorted(unreg_devices, key=operator.itemgetter(3),reverse=True)
#
# unreg_devices_count = MOD_cucm_funcs.cucm_count_interering_device(unreg_devices)
#
